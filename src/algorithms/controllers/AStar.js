...
export default {
  ...
  run(chunker, {edgeValueMatrix, coordsMatrix, startNode, endNodes, heuristicFn, moveNode, edgeCostType}) {
...
colors.FRONTIER_E);
          ...
          let weight = edgeValueMatrix[currentVertex][m];

          let costToAdd = weight;
            if (edgeCostType === "distance") {
              costToAdd = weight;
            } else if (edgeCostType === "euclidean") {
              const [x1, y1] = coords[currentVertex];
              const [x2, y2] = coords[m];
              costToAdd = Math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2);
            } else if (edgeCostType === "unit") {
              costToAdd = 1;
            }
          
          const newCost = cost[currentVertex] + costToAdd;

          console.log("edgeCostType:", edgeCostType, "u:", currentVertex, "v:", m, "weight:", weight, "costToAdd:", costToAdd);
