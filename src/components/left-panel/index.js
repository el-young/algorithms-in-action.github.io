...
import { DEFAULT_ALGORITHM, GlobalActions } from '../../context/actions';
import '../../styles/LeftPanel.scss';
import { DeployedAlgorithmCategoryList, DeployedAlgorithmList, getCategory } from '../../algorithms/masterList';
import { setFontSize } from '../top/helper';
import { INSTRUCTIONS_INDEX, PSEUDOCOE_INDEX } from '../right-panel';

// Do not load site with categories expanded
const LIST_COLLAPSE = false;

function LeftPanel({ fontSize, fontSizeIncrement, setRightTab }) {
  ...
  // On click of button check if the category has been seen
  // if so do not show instruction in right pane else do.
  const seenCatsRef = React.useRef(new Set());

  // Add algorithm category from current URL, users can use URLs
  // to load a specific algortithm.
  useEffect(() => {
    const url = new URL(window.location.href);
    // Default algorithm, sometimes URL wont have alg param in which
    // case it is esentially given whatever DEFAULT_ALGORITHM is assigned
    const algFromUrl = url.searchParams.get("alg") || DEFAULT_ALGORITHM;

    const cat = getCategory(algFromUrl);
    if (cat) {
      seenCatsRef.current.add(cat);
      setRightTab(INSTRUCTIONS_INDEX);
    }
  }, []);

  const onPickAlgo = (algo) => {
    // Do not load algorithm again if its already loaded
    // code base is not built to handle this.
    const same =
      algorithm?.id?.name === algo.shorthand &&
      algorithm?.id?.mode === algo.mode;
    if (same) return;

    dispatch(GlobalActions.LOAD_ALGORITHM, { name: algo.shorthand, mode: algo.mode });

    const cat = getCategory(algo.shorthand);
    if (!seenCatsRef.current.has(cat)) {
      setRightTab(INSTRUCTIONS_INDEX);
      seenCatsRef.current.add(cat);
    } else {
      setRightTab(PSEUDOCOE_INDEX);
    }
  };
